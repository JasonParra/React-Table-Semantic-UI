...
class CustomTable extends PureComponent {
  ...
  renderHeaders = () => {
    ...
    return <Table.Header >
      <Table.Row>
        {(headers || []).map(item =>
          <Table.HeaderCell onClick={() => this.handleSort(item)}>
  ...
  handleFooter = (i) => {
    ...
    if (i >= 0 && i < pagesQ)
      this.setState({
        pageSelection: i === 0 ? [0, defaultPages]
          : [i * defaultPages, ((i * defaultPages) + defaultPages)],
        page: i
      })
  }
  ...
  renderData = () => {
    ...
    return <Table.Body>
      {(_data || []).map(item => <Table.Row>
        {(labels || []).map(label => <Table.Cell>{item[label] || ""}</Table.Cell>)}
      </Table.Row>)
      }
    </Table.Body>
  }
